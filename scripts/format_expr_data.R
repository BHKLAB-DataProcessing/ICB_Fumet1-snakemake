...
seq_sample <- read.csv(file.path(input_dir, "filereport_read_run_PRJNA786567_tsv.txt"), sep = "\t")
...
unzip(file.path(input_dir, "ICB_Fumet1_RNAseq.zip"), exdir = file.path(input_dir, "rnaseq"))
...
for (name in names(expr_list)) {
  ...
  write.table(expr_df, file = file.path(output_dir, paste0("ICB_Fumet1_", name, ".tsv")), row.names = TRUE, col.names = TRUE, sep = "\t")
}
